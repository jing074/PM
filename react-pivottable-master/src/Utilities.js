...
class PivotData {
  constructor(inputProps = {}) {
    ...
    PivotData.forEachRecord(
      ...
      record => {
        if(this.props.isMultiVal)
        {
          for(const valIndex in this.props.vals)
          {
            if (this.filter(record)) {
             this.processRecord(record,this.props.vals[valIndex]);
            }
          }
        }
        else
  ...
  arrSort(attrs,col) {
    ...
    if(col)
    {
    sortersArr.push(sortAs(this.props.vals));
    }
    return function(a, b) {
  ...
  processRecord(record,val) {
    ...
    if (colKey.length !== 0) {
      ...
      this.colTotals[flatColKey].push(record,val);
    }
    ...
    if (colKey.length !== 0 && rowKey.length !== 0) {
      ...
      this.tree[flatRowKey][flatColKey].push(record,val);
    }
  }
